...
class DNS_Over_X:
    def __init__(self,debug):
        ...
        #self.__scan_data = {port:[] for port in self.__freeports}
        self.__scan_map = {}
    ...
    def init_tcp_connections(self):
        ...
        for i in tqdm(range(len(self.__ip_list))):
            ...
            self.__scan_data[port] = [(i,datetime.utcnow(),ip,port,seq,seq,"S","")]
            self.__scan_map[(port,seq)] = [ip]
    ...
    def process_packet_data(self,data):
        # load packet history of this port
        history = self.__scan_data[data[2]] #dport
        # if history is empty -> retransmission packet, not in use, skip this
        if not history:
            return
        if self.__debug:
        ...
        if data[3] == "SA" and not data[6]:
            ...
            self.__scan_data[data[2]] = history
            if self.__debug:
                #print(f"{my_colors.SEND}[*] Sending Ack with DNS to:\n->ip={data[1]}\n->port={data[2]}\n->seq={data[5]}\n->ack={data[4]+1}{my_colors.END}")
                print(f"{my_colors.SEND}[*] Sending Ack with DNS to:\n->ip={history[0][2]}\n->port={data[2]}\n->seq={data[5]}\n->ack={data[4]+1}{my_colors.END}")
            #self.send_ack_with_dns(data[1],data[2],data[4],data[5])
            self.send_ack_with_dns(history[0][2],data[2],data[4],data[5])
        elif data[3] == "FA" or data[3] == "F":
            # put in write back queue
            self.__writeback.put(data[2])
            ...
            self.send_ack_or_fin_packet(data[1],data[2],data[4],data[5],"F")
        else:
            if self.__debug:
    ...
    def writeback_data(self,port):
        #print(f"[*] Port: {port}")
        with open(self.__config["log_file"],"a") as log:
            pkt_history = self.__scan_data[port]
            ...
            if pkt_history:
                for pkt in pkt_history:
                    ...
                    data_line = ""
        # empty data history
        self.__scan_data[port] = None
        # free port
        if port not in self.__freeports:
            self.__freeports.append(port)
            self.__port_sem.release()
    ...
    def timeout_thread(self):
        while not self.__all_pkts_sent:
            ...
            for port in self.__scan_data.keys():
                timestamp = datetime.utcnow()
                data = self.__scan_data[port]
                if data:
                    #if self.__debug:
                    ...
                    if timedelta.total_seconds() > 10:
                        if self.__debug:
                            print(f"{my_colors.ERROR}[*] Timeout on {port=}.{my_colors.END}")
                        self.__writeback.put(port)
